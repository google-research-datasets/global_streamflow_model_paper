...
def spatial_plot(
    ...
    if title is not None:
        ax.set_title(
            title, 
            fontsize=NATURE_FONT_SIZES['one_half_column']['title']
        )
    ...
    if colorbar:
        ...
        cbar.set_label(
            metric, 
            rotation=90, 
            fontsize=NATURE_FONT_SIZES['one_half_column']['cbar']
        )
        cbar.ax.tick_params(labelsize=NATURE_FONT_SIZES['one_half_column']['c_tick_labels']) 
...
def plot_gdp_gauge_record_correlation(
    ...
    plt.xlabel(
        '(log) GDP in USD', 
        fontsize=NATURE_FONT_SIZES['one_column']['axis_label']
    )
    plt.ylabel(
    ...
    for country, data in df.dropna().iterrows():
        if country in countries_to_label:
            plt.text(
              ...
              fontdict=dict(
                  color='indigo', 
                  size=NATURE_FONT_SIZES['one_column']['text_labels']
              ),
            )
...
def hydrograph_metrics_cdf_plots(
    ...
    for ax, metric in zip(axes.flatten(), METRICS_AXIS_LIMITS):
        ...
        if gauges is not None:
            for experiment in data_paths.EXPERIMENTS:
                google_metrics_data[experiment] = google_metrics_data[experiment].loc[gauges]
